module.exports = (function() {
  ...
  ID.wrap = function(pointer) {
    // NOTE: Storing allocations in objc_getAssociatedObject and objc_setAssociatedObject creates
    // two issues, the first is we leak memory for items that have been collected by th GC. The 
    // second is at times the NSAutoreleasePool collects these objects adn they returns back as
    // NULL.  The approach/fix for this is a bit unclear, at present it seems this is used to 
    // help speed up the runtime. (See memtest.js in unit tests)
    ...
    //if (p.isNull()) {
      ...
      //r.free = false;
      //console.assert(typeof(rtn.pointer) != 'undefined' && rtn.potiner !== 0 && rtn.pointer.address() !== 0, 'return set obj has null pointer.');
      //r.writeObject(rtn, 0);
