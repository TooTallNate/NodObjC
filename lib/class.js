module.exports = (function() { 
  ...
  /**
   * This is depreciated and executes Class.addInstanceMethod
   * @see Class#addClassMethod
   * @see Class#addInstanceMethod
   * @depreciated
   */
  Class.prototype.addMethod = function(sel, type, func) {
    console.warn('DEPRECATED: Use addClassMethod if you wish to define a method available as a class method.');
    return this.addInstanceMethod(sel,type,func);
  }

  /**
   * Adds a new method to the specified class as a static or class method.  This method
   * is only usable on the class, not on its instance (e.g, +).
   */
  Class.prototype.addClassMethod = function(sel, type, func) {
    ...
    if (!core.class_addMethod(core.object_getClass(this.classPointer), selRef, funcPtr, typeStr))
  ...
  /**
   * Adds a new method to the specified class as an instance method (e.g., only usable
   * on an actual id or real object e.g, -).
   */
  Class.prototype.addInstanceMethod = function(sel, type, func) {
    var parsed = core.Types.parse(type);
    var selRef = core.unwrapValue(sel,':');
    var funcPtr = core.createWrapperPointer(func, parsed);
  ...
  Class.prototype.msgSend = function(sel, args, supre) {
        ...
        structRaw.size > 16) 
    {
      ...
      msgtypes[0] = 'v';
      msgtypes[1].unshift('?');
      args.unshift(structNew.ref());
    } 

    var msgfunctptr = supre ? ( structNew ? core.objc_msgSendSuper_stret 
                                          : core.objc_msgSendSuper )
                            : ( structNew ? core.objc_msgSend_stret
                                          : core.objc_msgSend );

    var msgSendFunc = core.createUnwrapperFunction(msgfunctptr,msgtypes);
    try {
      unwrappedVals = msgSendFunc.apply(null, args);
      if(!structNew) return unwrappedVals;
      else return structNew;
    } catch (e) {
      if (!e.hasOwnProperty('stack')) throw core.getException(e);
      else throw e;
    }
  }
  ...
  Class.prototype.getInstanceVariable = function(name) { return core.wrapValue(core.class_getInstanceVariable(this.classPointer, name)); }
