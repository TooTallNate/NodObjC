module.exports = (function() {
  ...
  function parseBridgeFile(fw, onto, recursive) {
    ...
    var tags = {'function':{'retval':{'retval':{},'arg':{}},'arg':{}},'depends_on':{},'string_constant':{},'enum':{},'struct':{},'constant':{}};
    var found = [];

    function parseAttrib(tag,tagcontent) {
      var attr = unescape(tagcontent.substring(1+tag.name.length,tagcontent.indexOf('>'))).trim();
      if(attr[attr.length-1] == '/') attr = attr.substring(0, attr.length-1);
      attr = attr.split('=\'');
      var coll = {};
      for(var i=1; i < attr.length; i++) {
        var name = attr[i-1].substring(attr[i-1].lastIndexOf(' ')).trim();
        var val = attr[i].substring(0,attr[i].lastIndexOf('\'')).replace(new RegExp("&quot;","g"),'"') 
        coll[name] = val;        
      }
      return coll;
    }

    function parseTag(tagNames, tag, content) {
      if(content.trim() == "" || tagNames === true) return [];
      var tagend2 = content.indexOf('</'+tag.name+'>', tag.start);
      var tagend = tagend2 != -1 ? tagend2 : content.indexOf('/>',tag.start);
      tag.end = tagend + 2 + ((tagend2 != -1) ? (tag.name.length + 1) : 0);
      var tagcontent = content.substring(tag.start, tag.end);
      tag.attrib = parseAttrib(tag,tagcontent);
      var tagbody = tagcontent.substring(tagcontent.indexOf('>')+1,tagcontent.indexOf("</"+tag.name)).trim();
      tag.children = findTags(tagNames[tag.name], tagbody);
      /*if(tag.attrib.name == "NSGetUncaughtExceptionHandler") {
        console.log('tag: ',tag);
        tag.children.forEach(function(e, z) { console.log(""); console.dir(e); console.log("");
          e.children.forEach(function(q) { console.dir(q); })
         })
      }*/
      return tag;
    }

    function findTags(tagNames, content) { 
      var ndx = 0, i = 0, tagKeys = Object.keys(tagNames), ftags=[];
      do {
        content = content.substring(ndx);
        for(i=0; i < tagKeys.length; i++) {
          var name = content.substring(1,tagKeys[i].length+1);
          if(name == tagKeys[i])
            ftags.push(parseTag(tagNames,{start:1, name:tagKeys[i]},content));
        }
        ndx = content.indexOf('<', ftags.length ? ftags[ftags.length-1].end : 1);
      } while(ndx != -1);
      return ftags;
/*
      if(!tagNames) return [];
      var tags = [];
      var tagKeys = Object.keys(tagNames);


      tagKeys.forEach(function(e) { 
        var ndxs = [];
        while(true) { 
          var start = content.indexOf('<'+e, ndxs.length == 0 ? 0 : ndxs[ndxs.length - 1].end);
          if(start == -1) break;
          ndxs.push(parseTag(tagNames,{start: start, name:e},content));
        }
        tags = tags.concat(ndxs);
      });
      return tags.sort(function(a,b) { return a.start > b.start; });*/
    }

    var nodes = findTags(tags, contents);
    ...
    nodes.forEach(function (node) {
      ...
      switch (node.name) {
        ...
        case 'string_constant':
          ...
          break;
        case 'enum':
          var ignore = node.attrib['ignore'];
          if (ignore !== "true") onto[name] = Number(getValue(node));
          break;
        case 'struct':
        ...
        case 'function':
          ...
          Object.defineProperty(onto, name, {
            get:function() {
              ...
              if (isInline)
                assert.ok(fw.inline, name+', '+fw.name+': declared inline but could not find inline dylib!',fw,isInline);
  ...
  ex.parseBridgeFile = parseBridgeFile;
  return ex;
})();