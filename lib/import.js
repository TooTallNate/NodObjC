module.exports = (function() {
  ...
  function parseBridgeFile(fw, onto, recursive) {
    ...
    var tags = {'function':{'retval':{'retval':null,'arg':null},'arg':null},'depends_on':null,'string_constant':null,'enum':null,'struct':null,'constant':null};
    var found = [];
    ...
    function parseAttrib(tag,attr) {
      ...
      for(var i=1; i < attr.length; i++) {
        var name = attr[i-1].substring(attr[i-1].lastIndexOf(' ')+1);
        var val = attr[i].substring(0,attr[i].lastIndexOf('\'')).replace(quoteRegExp,'"');
        coll[name] = val;        
      }
      return coll;
    }
    ...
    function parseTag(names, tag, content) {
      var sattr = 2+tag.name.length
          , eattr = content.indexOf('>',1)
          , isBodyless = (content[eattr-1]=='/');
      
      var sbody = eattr+1
        , ebody = isBodyless ? eattr-1 : content.indexOf('</'+tag.name, 1); 
      
      tag.end = isBodyless ? ebody + 2 : ebody + tag.name.length + 3;
      tag.attrib = parseAttrib(tag,content.substring(sattr,eattr));
    ...
    function findTags(names, content) { 
      var ndx = 0, i = 0, tagKeys = Object.keys(names), ftags=[];
      do {
        content = content.substring(ndx);
        for(i=0; i < tagKeys.length; i++) {
          // quick break for non-matching keys.
          if(content[1] == tagKeys[i][0] && content[2] == tagKeys[i][1]) {
            var name = content.substring(1,tagKeys[i].length+1);
            if(name == tagKeys[i]) ftags.push(parseTag(names,{name:tagKeys[i]},content));
          }
        }
