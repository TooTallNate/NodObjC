module.exports = (function() {
 ...
 /**
   * Creates and/or returns an appropriately wrapped up 'objc_msgSendSuper_stret'
   * function based on the given Method description info.
   * TODO: Move this into a node-ffi helper class thingy.
   */
  function get_objc_msgSendSuper_stret (objcTypes) {
    var type = []
      , rtn = [ types.map(objcTypes[0]), type ]
      , args = objcTypes[1]
      , i = 0
    ...
    var key = rtn.toString();
    // first check the cache
    if (msgSendCacheSuperStret[key]) return msgSendCacheSuperStret[key];
    // If we got here, then create a new objc_msgSend ffi wrapper
    // TODO: Don't use the Library helper, use ffi low-level API
    var lib = new ffi.Library(null, { objc_msgSendSuper_stret: rtn });
    // return and cache at the same time
    return msgSendCacheSuperStret[key] = lib.objc_msgSendSuper
  }
