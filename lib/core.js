module.exports = (function() {
  ...
  function copyIvarList (classPtr) {
    var rtn = []
      , ivars = class_copyIvarList(classPtr, ref.alloc('uint'))
      , count = numIvars.deref();

    for (var i=0; i<count; i++) 
      rtn.push(ivar_getName(ivars.readPointer(i * ref.sizeof.pointer)));
  ...
  function copyMethodList (classPtr) {
    var numMethods = ref.alloc('uint')
      , rtn = []
      , methods = class_copyMethodList(classPtr, numMethods)
      , count = numMethods.deref();

    for (var i=0; i<count; i++)
      rtn.push(SEL.toString(method_getName(methods.readPointer(i * ref.sizeof.pointer))));
  ...
  function copyMethodDescriptionList (protocolPtr, required, instance) {
    var numMethods = ref.alloc('uint')
      , methods = protocol_copyMethodDescriptionList(protocolPtr, required, instance, numMethods)
      , rtn = []
      , p = methods
      , count = numMethods.deref();
   ...
   */
  function get_objc_msgSend (objcTypes) {
    var type = []
    ...
    var lib = new ffi.Library(null, { objc_msgSend: rtn });
   ...
   * TODO: Move this into a node-ffi helper class thingy.
   */
  function get_objc_msgSend_stret (objcTypes) {
    var type = []
      , rtn = [ types.map(objcTypes[0]), type ]
      , args = objcTypes[1]
      , i = 0
    ...
    for (; i<l; i++) {
      if(i==0) type.push('pointer');
      else type.push(types.map(args[i]));
    }
    // Stringify the types
    var key = rtn.toString();
    // first check the cache
    if (msgSendStretCache[key]) return msgSendStretCache[key];

    // If we got here, then create a new objc_msgSend ffi wrapper
    // TODO: Don't use the Library helper, use ffi low-level API
    var lib = new ffi.Library(null, { objc_msgSend_stret: rtn });

    // return and cache at the same time
    return msgSendStretCache[key] = lib.objc_msgSend_stret
  }
   ...
   * TODO: Move this into a node-ffi helper class thingy.
   */
  function get_objc_msgSendSuper (objcTypes) {
    var type = []
      , rtn = [ types.map(objcTypes[0]), type ]
      , args = objcTypes[1]
      , i = 0
    ...
    // If we got here, then create a new objc_msgSend ffi wrapper
    // TODO: Don't use the Library helper, use ffi low-level API
    var lib = new ffi.Library(null, { objc_msgSendSuper: rtn });
    // return and cache at the same time
    return msgSendCacheSuper[key] = lib.objc_msgSendSuper
  }
  ...
  function wrapValue (val, type) {
    ...
    if (type.function_pointer && strtype == '@?')
      rtn = createBlock(val, type);
    else if(type.function_pointer && strtype != '@?')
      rtn = createUnwrapperFunction(val, type);
    else if (strtype == '@') {
      var ID = require('./id');
      //TODO: Decoule this...
      rtn = ID.wrap(val);
  ...
  ex.get_objc_msgSend = get_objc_msgSend;
  ex.get_objc_msgSend_stret = get_objc_msgSend_stret;
  ex.get_objc_msgSendSuper = get_objc_msgSendSuper;
  ex.objc_method_description = struct({ name: 'pointer', types: 'string' });
  ex.wrapValue = wrapValue;
  ex.wrapValues = wrapValues;
  ex.unwrapValues = unwrapValues;
  ex.unwrapValue = unwrapValues;
  return ex;
})();