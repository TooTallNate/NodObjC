module.exports = (function() {
  ...
  function wrapValue (val, type) {
    ...
    if (strtype == '@?')
      ...
      var wrappedObj, cache = objc.objc_getAssociatedObject(val, objc.objcStorageKey);
  ...
  function unwrapValue (val, type) {
    var strtype = type.type ? type.type : type;
    if (strtype == '@?') {
      if ('pointer' in val) return val.pointer;
      else return createBlockPointer(blockOrFunc, type);
      return getBlockPointer(val, type);
    } else if (strtype == '^?')
      ...
      var rtn = selCache[val];
      if (rtn) return rtn;
      return selCache[val] = objc.sel_registerName(val);
    } else if (val === true) {
      return 1;
    } else if (val === false) {
      return 0;
  ...
  function createBlock (func, type) {
    return wrapValue(createBlockPointer(func, type),'@');
  }

  /*!
   * Module exports.
   */
  objc.parseArgs = parseArgs;
