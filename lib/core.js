module.exports = (function() {
  ...
  function createObject (val, type, name) {
    ...
    var w = (type == '@') ? new (require('./id'))(val)
                          : new (require('./class'))(val);

    var rtn = name ?
      (new Function('w', 'return function ' + name + ' () { ' +
        'return ' + name + '.msgSend(arguments, false); }'))(w) :
      function () {
        return rtn.msgSend(arguments, false, rtn);
      };

    setPrototypeOf(rtn, w);
